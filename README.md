### 🗺️ Карта навигации: Общая структура (Иерархия)

<pre>
meta-platform/ <-- Общий размер (в байтах): 0
├── meta/
│   ├── dte/ <-- Модули управления компонентами M^DTE (^A/S)
│   │   ├── app/ 🟡 <-- Этот компонент необходим для скачивания исключительно прикладным разработчикам
│   │   │   ├── ui/
│   │   │   │   ├── cli/
│   │   │   │   │   ├── adte.c 🔐
│   │   │   │   │   └── adte.exe ⌨️
│   │   │   │   └── gui/
│   │   │   │       ├── <s>adte.c 🔐</s>
│   │   │   │       └── <s>adte.exe 🖱️</s>
│   │   │   ├── <s>adte-cli.c 🔐</s>
│   │   │   ├── <s>adte-cli.exe ⌨️</s>
│   │   │   ├── <s>adte-gui.c 🔐</s>
│   │   │   └── <s>adte-gui.exe 🖱️</s>
│   │   ├── sys/ 🔴 <-- Этот компонент необходим для скачивания исключительно системным разработчикам
│   │   │   ├── ui/
│   │   │   │   ├── cli/
│   │   │   │   │   ├── sdte.c 🔐
│   │   │   │   │   └── sdte.exe ⌨️
│   │   │   │   └── gui/
│   │   │   │       ├── <s>sdte.c 🔐</s>
│   │   │   │       └── <s>sdte.exe 🖱️</s>
│   │   │   ├── <s>sdte-cli.c 🔐</s>
│   │   │   ├── <s>sdte-cli.exe ⌨️</s>
│   │   │   ├── <s>sdte-gui.c 🔐</s>
│   │   │   └── <s>sdte-gui.exe 🖱️</s>
│   │   ├── ui/
│   │   │   ├── cli/
│   │   │   │   ├── dte.c 🔐
│   │   │   │   └── dte.exe ⌨️ 🟣 <-- Этот компонент необходим для скачивания как прикладным, так и системным разработчикам
│   │   │   └── gui/
│   │   │       ├── <s>dte.c 🔐</s>
│   │   │       └── <s>dte.exe 🖱️</s> 🟣 <-- Этот компонент необходим для скачивания как прикладным, так и системным разработчикам
│   │   ├── <s>dte-cli.c 🔐</s>
│   │   ├── <s>dte-cli.exe ⌨️</s> 🟣 <-- Этот компонент необходим для скачивания как прикладным, так и системным разработчикам
│   │   ├── <s>dte-gui.c 🔐</s>
│   │   └── <s>dte-gui.exe 🖱️</s> 🟣 <-- Этот компонент необходим для скачивания как прикладным, так и системным разработчикам
│   └── re/ 🟢 <-- Этот компонент необходим для скачивания исключительно конечному пользователю
│       ├── re.c 🔐
│       └── re.exe ⌨️  # Плеер для запуска приложений (невидимо вызывается самим приложением, которое было запущено)
│                      # Проверяет цифровую подпись и принадлежность к экосистеме мета-платформы
├── mde.c 🔐
└── mde.exe ⌨️ <-- Оркестратор ─ диспетчер задач мета-платформы (Managed Development Environment)
</pre>

#### Обозначения:

<pre>
🔐: Закрытый исходный код
<s>⚫: [🇺🇸] ─ / [🇷🇺] ─</s>
🔴: [🇺🇸] Meta System Development Tools Environment / [🇷🇺] Мета Среда Инструментов Системной Разработки
🟣: [🇺🇸] Two-in-one / [🇷🇺] Два в одном</s>
🟡: [🇺🇸] Meta Application Development Tools Environment / [🇷🇺] Мета Среда Инструментов Прикладной Разработки
🟢: [🇺🇸] Meta Runtime Environment (MRE) / [🇷🇺] Мета Среда Выполнения (МСВ)
<!--📟/-->⌨️: [🇺🇸] Command Line Interface (CLI) / [🇷🇺] Интерфейс командной строки (ИКС)
<!--🖼️/🎨/-->🖱️: [🇺🇸] Graphical User Interface (GUI) / [🇷🇺] Графический интерфейс пользователя (ГИП)
<!--
meta-platform/
├── meta/
│   ├── re/ 🟢
│   │   ├── re.c 🔐
│   │   └── re.exe ⌨️    <-- [🇺🇸] Meta Runtime Environment / [🇷🇺] Мета Среда Выполнения
│   ├── adte/ 🟡
│   │   ├── adte.c 🔐
│   │   ├── adte.exe ⌨️  <-- [🇺🇸] Meta Application Development Tools Environment / [🇷🇺] Мета Среда Инструментов Прикладной Разработки
│   │   <s>└── adte-gui.exe 🖱️</s>
│   └── sdte/ 🔴
│       ├── sdte.c 🔐
│       ├── sdte.exe ⌨️  <-- [🇺🇸] Meta System Development Tools Environment / [🇷🇺] Мета Среда Инструментов Системной Разработки
│       <s>└── sdte-gui.exe 🖱️</s>
├── mde.c 🔐
└── mde.exe ⌨️           <-- Оркестратор ─ сердце системы (Managed Development Environment)
-->
</pre>