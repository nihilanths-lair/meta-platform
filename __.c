...
int main()
{
    ...
    case 133: cache[cache[ip+1]]  = cache[ip+2]; ip+=3; goto exec; // записать в произвольную ячейку памяти (трёхбайтовая операция)
    case 134: cache[cache[ip+1]] += cache[ip+2]; ip+=3; goto exec; // добавить к произвольной ячейки памяти (трёхбайтовая операция)
    case 135: cache[cache[ip+1]] -= cache[ip+2]; ip+=3; goto exec; // убавить из произвольной ячейки памяти (трёхбайтовая операция)
    default: printf("\n Неизвестный опкод.");
    }
    proc_exit:
    printf("\n Эмуляция окончена.\n");
