...
int main()
{
    ...
    // #1
    case '<': dp--; ip++;                               goto exec; // 1 | перейти к предыдущей ЯП данных
    case '>': dp++; ip++;                               goto exec; // 1 | перейти к следующей ЯП данных
    // extented: JMP-- / JMP++ (декремент/инкремент потока выполнения)
    case 4: ip--; goto exec; // использовать только в режиме ручного управления (для избежания багов)
    case 5: ip++; goto exec; // использовать только в режиме ручного управления (для избежания багов)
    // extented: 2 - JMP # (плавающий указатель данных/кода)
    case '~': dp = cache[ip+1]; ip+=2;                  goto exec; // 2 | перейти к произвольной (абсолютной) ЯП данных
    case 6: ip = cache[ip+1];                           goto exec; // 2 | перейти к произвольной (абсолютной) ЯП кода
    // #2
    ...
    case 129: cache[ip] -= cache[ip+1];                 goto exec; // убавить из текущей ячейки памяти (двухбайтовая операция)
    // #4
    case 130: cache[cache[ip+1]]  = cache[ip+2];        goto exec; // записать в произвольную ячейку памяти (трёхбайтовая операция)
    case 131: cache[cache[ip+1]] += cache[ip+2];        goto exec; // добавить к произвольной ячейки памяти (трёхбайтовая операция)
    case 132: cache[cache[ip+1]] -= cache[ip+2];        goto exec; // убавить из произвольной ячейки памяти (трёхбайтовая операция)
    // #5
    case 133: ip += cache[ip+1];                        goto exec; // перейти к произвольной ячейки памяти с вектором направления вперёд (двухбайтовая операция)
    case 134: ip -= cache[ip+1];                        goto exec; // перейти к произвольной ячейки памяти с вектором направления назад (двухбайтовая операция)
    case 135: ip = cache[ip+1];                         goto exec; // перейти к произвольной ячейки памяти (двухбайтовая операция)
    */
