...
int main()
{
    ...
    switch (cache[ip]){
    case '+': cache[ip]++;              ip--;  goto exec; // инкремент текущей ячейки памяти (однобайтовая операция)
    case '-': cache[ip]--;              ip--;  goto exec; // декремент текущей ячейки памяти (однобайтовая операция)
    case '>': ip++;                     ip--;  goto exec; // перейти к следующей ячейки памяти (однобайтовая операция)
    case '<': ip--;                     ip--;  goto exec; // перейти к предыдущей ячейки памяти (однобайтовая операция)
    case '=': cache[ip] = cache[ip+1];  ip-=2; goto exec; // записать в текущую ячейку памяти (двухбайтовая операция)
    case 128: cache[ip] += cache[ip+1]; ip-=2; goto exec; // добавить к текущей ячейки памяти (двухбайтовая операция)
    case 129: cache[ip] -= cache[ip+1]; ip-=2; goto exec; // убавить из текущей ячейки памяти (двухбайтовая операция)
    case 130: ip += cache[ip+1];               goto exec; // перейти к произвольной ячейки памяти с вектором направления вперёд (двухбайтовая операция)
    case 131: ip -= cache[ip+1];               goto exec; // перейти к произвольной ячейки памяти с вектором направления назад (двухбайтовая операция)
    case 132: ip = cache[ip+1];                goto exec; // перейти к произвольной ячейки памяти (двухбайтовая операция)
    default: printf("\n Неизвестный опкод."); goto proc_exit;
    ...
    switch (cache[ip]){
    /*
    // 1. Быстрые однобайтовые операции (Концепция DP)
    case '+': cache[dp]++; ip++; goto exec; // инкремент текущей ячейки памяти (однобайтовая операция)
    case '-': cache[dp]--; ip++; goto exec; // декремент текущей ячейки памяти (однобайтовая операция)
    case '>': dp++;        ip++; goto exec; // перейти к следующей ячейки памяти (однобайтовая операция)
    case '<': dp--;        ip++; goto exec; // перейти к предыдущей ячейки памяти (однобайтовая операция)
    */
